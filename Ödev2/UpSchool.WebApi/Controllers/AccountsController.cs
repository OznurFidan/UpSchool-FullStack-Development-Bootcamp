...
namespace UpSchool.WebApi.Controllers
{
    ...
    public class AccountsController : ControllerBase
    {
        ...
        {
            IQueryable<Account> accountsQuary=_dbContext.Accounts.AsQueryable();

            if(!string.IsNullOrEmpty(searchKeyword)) accountsQuary= accountsQuary.Where(x=>
            x.Title.Contains(searchKeyword)||x.UserName.Contains(searchKeyword));

            accountsQuary=isAscending
                ?accountsQuary.OrderBy(x=>x.Title)
                :accountsQuary.OrderByDescending(x=>x.UserName);
                


            var accounts =accountsQuary.ToList();
